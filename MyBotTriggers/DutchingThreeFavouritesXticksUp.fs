...
module DutchingThreeFavouritesXticksUp
...
/// <summary>
/// DutchingThreeFavouritesXticksUp
/// </summary>
type DutchingThreeFavouritesXticksUp(market : Market, selection : Selection, botName : string, botTriggerParameters : BotTriggerParameters, myBfexplorer : IMyBfexplorer) =
    ...
    member private this.PlaceDutchBets() =
        ...
        let oddsContext = OddsContext()
        let ticksUp = defaultArg (botTriggerParameters.GetParameter<int>("TicksUp")) 3

        dutchSelections |> List.iter (fun s -> s.Price <- oddsContext.GetOddsUp(s.Price, ticksUp))
